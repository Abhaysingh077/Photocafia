...
//===============menu================
	...
	@GetMapping("/menu")
	public List<Menu> getAllItems() throws ClassNotFoundException {
List<Menu> items= menuService.getAllItems();
		if(items.isEmpty()){
			throw new ClassNotFoundException("Menu Is Empty");
		}else{
			return items;
		}
	}
	...
	public List<Menu> getAllItem_names(@PathVariable String category) throws ClassNotFoundException {
	...
	@GetMapping("/mnu/{item}")
	public Menu getAllItem_name(@PathVariable String item) throws ClassNotFoundException {
		Menu itemByName = menuService.findAllItems(item);
		if(!itemByName.equals(null)){
			return itemByName;
		}else{

			throw new ClassNotFoundException("Food Items Doesn't Exist");
		}
	}
...
//	========= Cart=============
	...
	@GetMapping("/cart")
	public List<Cart> getAllIt() throws ClassNotFoundException {
		List<Cart> carts= menuService.getAllIt();
		if(carts.isEmpty()){
			throw new ClassNotFoundException("Cart Isd5 Empty");
		}else{
			return carts;
		}
	}
	...
	public List<Cart> findAll(@PathVariable("name") String name) throws ClassNotFoundException {
		List<Cart> cartByName= menuService.findAllIt(name);
		if(cartByName.isEmpty()){
			throw new ClassNotFoundException("Cart with this Name is not Exist");
		}else{
			return cartByName;
		}
